	...
	<Page :title="$page.props.lang.admin.manage + $page.props.lang.admin.measuring_units.toLowerCase()">
		...
		<table class="table table-striped">
			<thead>
				<tr>
					<th>
						<Link :class="{ active: sort === 'id' }" :href="route('unit.admin', { sort: 'id', order: order, page: page })">Id</Link>
						<span v-if="sort === 'id'" v-html="order==='asc' ? props.arrow.down : props.arrow.up"/>
					</th>
					<th>
						<Link :class="{ active: sort === 'name' }" :href="route('unit.admin', { sort: 'name', order: order, page: page })">{{ $page.props.lang.admin.name }}</Link>
						<span v-if="sort === 'name'" v-html="order==='asc' ? props.arrow.down : props.arrow.up"/>
					</th>
					<th>
						<Link :class="{ active: sort === 'type' }" :href="route('unit.admin', { sort: 'type', order: order, page: page })">{{ $page.props.lang.admin.type }}</Link>
						<span v-if="sort === 'type'" v-html="order==='asc' ? props.arrow.down : props.arrow.up"/>
					</th>
					<th>
						<Link :class="{ active: sort === 'is_enabled' }" :href="route('unit.admin', { sort: 'is_enabled', order: order, page: page })">{{ $page.props.lang.admin.enabled }}</Link>
						<span v-if="sort === 'is_enabled'" v-html="order==='asc' ? props.arrow.down : props.arrow.up"/>
					</th>
					<th>
						<Link :class="{ active: sort === 'created_at' }" :href="route('unit.admin', { sort: 'created_at', order: order, page: page })">{{ $page.props.lang.admin.created }}</Link>
						<span v-if="sort === 'created_at'" v-html="order==='asc' ? props.arrow.down : props.arrow.up"/>
					</th>
					<th>
						<Link :class="{ active: sort === 'updated_at' }" :href="route('unit.admin', { sort: 'updated_at', order: order, page: page })">{{ $page.props.lang.admin.updated }}</Link>
